...
class DataSet:
    def __init__(self, datapath='../data', frac=1, adddim=False, center=False,
                 pca_comps=None, poly=None,
        ...
        if frac:
            training_data = training_data.sample(
                frac=frac)  # randomly rearrange rows when 1
        if balance:
        ...
        if train_on_valid:
            ...
            self.training_data = pd.concat([self.training_data,
                                            self.prediction_data.dropna()]
                                           ).reset_index(drop=True)

        features = [f for f in list(self.training_data.columns) if
                    "feature" in f]
        ...
        if train_on_noise:
            ...
            training_with_noise[features] = self.training_data[features] \
                + extra_noise

            self.training_data = self.training_data.append(
                training_with_noise).reset_index(drop=True)
        ...
        if self.pca_comps:
            ...
            # print('pca explvar ratio:', self.pca.explained_variance_ratio_)

        if poly:
            self.poly = PolynomialFeatures(poly, interaction_only=True,
                                           include_bias=False)
        ...
        if pca_comps or poly:
            ...
            training_data_features = self.X_train.reshape(
                len(self.training_data), self.X_train.shape[1])
            featcols = ['feature_prepro_' + str(x)
            ...
            self.training_data = pd.concat(
                [self.training_data[['id', 'era', 'target']],
                 training_data_features], axis=1)

            # update prediction data to reflect this
            prediction_data_features = self.x_prediction.reshape(
                len(self.prediction_data), self.x_prediction.shape[1])
            prediction_data_features = pd.DataFrame(
                prediction_data_features,
                columns=['feature_prepro_' + str(x)
                         for x in range(self.x_prediction.shape[1])]
            )

            self.prediction_data = pd.concat(
                [self.prediction_data[['id', 'era', 'target', 'data_type']],
